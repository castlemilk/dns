...
func NewStatus(maxStaleness time.Duration) *Status {
	return &Status{maxStaleness: maxStaleness, now: time.Now}
}
...
func (c *Consumer) LoadCache() error {
	c.applyMu.Lock()
	...
	if err := c.applyLocked(raw, false, false); err != nil {
...
func (c *Consumer) Fetch(ctx context.Context) error {
	...
	if err := c.applyLocked(raw, true, true); err != nil {
...
func (c *Consumer) applyLocked(raw []byte, persist, fetched bool) error {
