...
func TestNewValidatesOptions(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "https with a token", opts: client.Options{BaseURL: "https://api.simple.test", Token: theSecret}},
		{
			name:    "empty url",
			opts:    client.Options{},
...
func TestNewNormalisesTheBaseURL(t *testing.T) {
	...
	clients, err := client.New(client.Options{BaseURL: "https://api.simple.test/"})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	if clients.BaseURL != "https://api.simple.test" {
...
func TestExplain(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: "the control plane refused the credential; run `simple auth login`, or pass --token",
		},
		{
		...
		},
		{
			...
			err:  fmt.Errorf("dial: %w", &net.DNSError{Err: "no such host", Name: "api.simple.invalid", IsNotFound: true}),
			want: `the control plane's host name "api.simple.invalid" could not be resolved`,
		},
		{
			name: "a refused dial",
...
func TestExplainFor(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "nil", target: "https://api.simple.test", err: nil, want: ""},
		{
			name:   "unreachable names the target",
			target: "https://api.simple.test",
			err:    connect.NewError(connect.CodeUnavailable, errors.New("refused")),
			want:   "https://api.simple.test is not reachable; check --api-url and that it is running",
		},
		{
			name:   "unauthenticated names the target",
			target: "https://api.simple.test",
			err:    connect.NewError(connect.CodeUnauthenticated, errors.New("unauthorized")),
			want:   "https://api.simple.test refused the credential; run `simple auth login`, or pass --token",
		},
		{
