// Package config holds the `simple` CLI's on-disk settings and the rule for
// combining them with flags and the environment.
//
// The file lives at ~/.config/simple/config.json, mode 0600 in a 0700
...
//
// Precedence is flag > environment (SIMPLE_API_URL, SIMPLE_API_TOKEN) > file,
...
const (
	...
	EnvAPIURL = "SIMPLE_API_URL"
	EnvToken  = "SIMPLE_API_TOKEN" //nolint:gosec // the name of a variable, not a credential
...
// Origin names where a resolved value came from, so `simple auth status` can
...
// read-only callers and `simple auth login` both run without one.
...
// unset the path is exactly ~/.config/simple/config.json.
func Path(getenv func(string) string) (string, error) {
	...
	if base == "" {
		...
		if home == "" {
			return "", errors.New("HOME: not set, so ~/.config/simple/config.json cannot be located")
		}
	...
	return filepath.Join(base, "simple", "config.json"), nil
}
...
func Load(path string) (Config, error) {
	...
	var stored Config
	// Unknown keys are ignored rather than rejected: `simple` and `simple-mcp`
...
// `simple auth logout` is idempotent.
