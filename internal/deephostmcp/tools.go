...
var domainSelector = map[string]any{
	...
	"zone_id": str("The zone id from deephost_domain_list. Either this or domain is required."),
}
...
var registry = []tool{
	...
	{
		Name:       "deephost_domain_add",
		Summary:    "Create a zone for a domain. This does not register the domain: the owner must still delegate it to the nameservers deephost_status reports.",
	...
	},
	{
		...
		Properties: fields(domainSelector, map[string]any{
			"record_id": str("The record id from deephost_record_list."),
	...
	},
	{
		...
		Properties:  fields(domainSelector, map[string]any{"record_id": str("The record id from deephost_record_list.")}),
		Required:    []string{"record_id"},
		Mutates:     true,
		Destructive: true,
	...
	{
		Name:    "deephost_site_deploy",
		Summary: "Deploy a domain's site from its git repository. Returns the queued deploy; poll deephost_site_show or deephost_site_deploy_list for the phase. Private repositories need a per-deploy token, which this server does not accept — use the `deephost site deploy` command for those.",
		Properties: fields(domainSelector, map[string]any{
	...
	{
		Name:    "deephost_site_deploy_list",
	...
	{
		Name:       "deephost_site_rollback",
		Summary:    "Put a previous deploy back in front of traffic. The deploy must be one this site released before.",
		Properties: fields(domainSelector, map[string]any{"deploy_id": str("The deploy id to return to, from deephost_site_deploy_list.")}),
		Required:   []string{"deploy_id"},
		Mutates:    true,
		Handler:    (*Server).siteRollback,
	},
	{
		Name:    "deephost_site_logs",
	...
	{
		Name:    "deephost_site_env_set",
		Summary: "Set one runtime environment variable on a site. The value is write-only: it is passed to the hosting engine and is never stored, logged or returned here, and it does not appear in the answer. The running site keeps its current values until the next deploy. Never put a secret in a chat transcript to get it here — set it with the `deephost site env set` command instead.",
		Properties: fields(domainSelector, map[string]any{
	...
	{
		Name:       "deephost_mail_mailbox_list",
	...
	{
		Name:    "deephost_mail_mailbox_add",
		Summary: "Create a mailbox and generate its first password. SECRET HANDLING: the password is in the answer, it is shown exactly once, and neither this server nor the platform can ever show it again. Give it to the person who owns the mailbox by a channel they already trust, tell them to change it, and do not write it to a file, a note, a ticket or a summary. If you cannot hand it over now, use deephost_mail_mailbox_reset later instead of keeping a copy.",
		Properties: fields(domainSelector, map[string]any{
	...
	{
		Name:    "deephost_mail_mailbox_reset",
		...
		Properties: fields(domainSelector, map[string]any{
			...
			"mailbox_id": str("The mailbox id from deephost_mail_mailbox_list. Either this or address is required."),
		}),
		Mutates: true,
		Handler: (*Server).mailboxReset,
	},
	{
		Name:        "deephost_mail_mailbox_delete",
	...
	{
		Name:        "deephost_mail_forwarder_delete",
		Summary:     "Delete a forwarder. Mail to its address stops being delivered.",
		Properties:  fields(domainSelector, map[string]any{"forwarder_id": str("The forwarder id from deephost_mail_forwarder_list.")}),
	...
	{
		Name:    "deephost_billing_subscribe",
	...
	{
		Name:    "deephost_billing_invoices",
