...
func TestEveryToolIsWellFormed(t *testing.T) {
	...
	for _, definition := range registry {
		t.Run(definition.Name, func(t *testing.T) {
			if !strings.HasPrefix(definition.Name, "deephost_") {
				t.Errorf("name %q does not mirror a `deephost` command", definition.Name)
			}
...
func TestDestructiveToolsAskForConfirmationInSchemaAndDescription(t *testing.T) {
	...
	for _, name := range []string{
		"deephost_domain_delete", "deephost_record_delete", "deephost_site_detach",
		"deephost_site_env_unset", "deephost_mail_unbind", "deephost_mail_mailbox_delete",
		"deephost_mail_forwarder_delete", "deephost_record_import",
	} {
...
func TestZoneIsIdentifiedByNameOrID(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			answer := client.call("deephost_domain_delete", arguments)
			if test.wantKind != "" {
...
func TestArgumentsAreRejectedByFieldName(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing required", tool: "deephost_record_add", arguments: map[string]any{"domain": "acme.dev", "type": "A", "value": "203.0.113.1"}, want: "name: required"},
		{name: "unknown record type", tool: "deephost_record_add", arguments: map[string]any{"domain": "acme.dev", "name": "@", "type": "PTR", "value": "x"}, want: "type: must be one of"},
		{name: "ttl is not a number", tool: "deephost_record_add", arguments: map[string]any{"domain": "acme.dev", "name": "@", "type": "A", "value": "203.0.113.1", "ttl": "soon"}, want: "ttl: must be a whole number"},
		{name: "unknown framework", tool: "deephost_site_attach", arguments: map[string]any{"domain": "acme.dev", "framework": "rails"}, want: "framework: must be one of"},
		{name: "unknown www mode", tool: "deephost_site_www", arguments: map[string]any{"domain": "acme.dev", "mode": "sideways"}, want: "mode: must be serve or redirect"},
		{name: "since is not a timestamp", tool: "deephost_activity_list", arguments: map[string]any{"since": "yesterday"}, want: "since: must be an RFC 3339 timestamp"},
		{name: "targets are not strings", tool: "deephost_mail_forwarder_add", arguments: map[string]any{"domain": "acme.dev", "local_part": "team", "targets": "mara@acme.dev"}, want: "targets: must be an array of strings"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			fakes := newPlatform()
...
func TestImportAsksForConfirmationOnlyWhenItReplaces(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			definition := toolNamed(t, "deephost_record_import")
			raw := args{}
