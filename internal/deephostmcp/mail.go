...
func (s *Server) findMailbox(ctx context.Context, zone zoneRef, a args, addressRequired bool) (*mailv1.Mailbox, error) {
	...
	if id != "" {
		return nil, notFound("mailbox_id: this domain has no mailbox with that id; call deephost_mail_mailbox_list to see the ones it has")
	}
	return nil, notFound("address: " + address + " is not a mailbox on this domain; call deephost_mail_mailbox_list to see the ones it has")
}
