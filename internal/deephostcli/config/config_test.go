...
func TestSaveWritesTightPermissions(t *testing.T) {
	...
	home := t.TempDir()
	path := filepath.Join(home, ".config", "deephost", "config.json")

	if err := config.Save(path, config.Config{APIURL: "https://api.deephost.test", Token: theSecret}); err != nil {
		t.Fatalf("Save: %v", err)
	}
	restore()
	...
	if loaded.Token != theSecret || loaded.APIURL != "https://api.deephost.test" {
...
func TestSaveOverwritesAndStaysAtomic(t *testing.T) {
	...
	path := filepath.Join(t.TempDir(), "deephost", "config.json")
...
func TestSaveRejectsBadValues(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			cfg:     config.Config{APIURL: "http://api.deephost.test"},
			wantErr: "api_url: must use https",
		},
		{
			name:    "url with a query",
			cfg:     config.Config{APIURL: "https://api.deephost.test?a=1"},
...
func TestResolvedValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "https anywhere is allowed", flags: config.Flags{APIURL: "https://api.deephost.test/"}, want: "https://api.deephost.test"},
		{
			name:    "userinfo is refused",
			flags:   config.Flags{APIURL: "https://user:pass@api.deephost.test"},
		...
		},
		{
			...
			flags:   config.Flags{APIURL: "https://api.deephost.test", Token: "bad token"},
			wantErr: "token: must contain only printable ASCII",
		},
	}
	for _, test := range tests {
...
func TestCredentialNeverRenders(t *testing.T) {
	...
	cfg := config.Config{APIURL: "https://api.deephost.test", Token: theSecret}
