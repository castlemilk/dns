...
func TestAuthorizeURL(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "https console",
			console:   "https://deephost.test/",
			callback:  "http://127.0.0.1:54321/callback",
			challenge: "chal",
			state:     "st",
			want:      "https://deephost.test/cli/authorize?callback=http%3A%2F%2F127.0.0.1%3A54321%2Fcallback&challenge=chal&state=st",
		},
		{
		...
		},
		{
			...
			console:   "http://deephost.test",
			callback:  "http://127.0.0.1:1/callback",
			challenge: "chal",
			state:     "st",
			wantErr:   "console_url: must use https",
		},
		{name: "missing callback", console: "https://deephost.test", challenge: "c", state: "s", wantErr: "callback: is required"},
		{name: "missing challenge", console: "https://deephost.test", callback: "http://127.0.0.1:1/callback", state: "s", wantErr: "challenge: is required"},
		{name: "missing state", console: "https://deephost.test", callback: "http://127.0.0.1:1/callback", challenge: "c", wantErr: "state: is required"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			credential, err := auth.Login(t.Context(), auth.LoginOptions{
				ConsoleURL: "https://deephost.test",
...
func TestLoginRejectsBadOptions(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "no store",
			opts:    auth.LoginOptions{ConsoleURL: "https://deephost.test"},
			wantErr: "store: is required",
		},
		{
			name: "an unusable console URL",
			opts: auth.LoginOptions{
				ConsoleURL: "ftp://deephost.test",
