...
func TestServeAnswersAReadOnlyToolOverAnInMemoryTransport(t *testing.T) {
	...
	answer := client.call("simple_domain_list", nil)
	if answer.IsError {
		t.Fatalf("simple_domain_list was refused: %v", answer.StructuredContent)
	}
...
func TestServeRefusesADestructiveToolWithoutConfirmation(t *testing.T) {
	...
	answer := client.call("simple_domain_delete", map[string]any{"domain": "acme.dev"})
	...
	// gate and not a wall.
	confirmed := client.call("simple_domain_delete", map[string]any{"domain": "acme.dev", "confirm": true})
...
func TestMutatingToolsRefuseWithoutACredential(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "create", tool: "simple_domain_add", arguments: map[string]any{"name": "new.dev"}},
		{name: "destructive", tool: "simple_domain_delete", arguments: map[string]any{"domain": "acme.dev", "confirm": true}},
		{name: "record", tool: "simple_record_add", arguments: map[string]any{"domain": "acme.dev", "name": "@", "type": "A", "value": "203.0.113.1"}},
		{name: "mailbox", tool: "simple_mail_mailbox_add", arguments: map[string]any{"domain": "acme.dev", "local_part": "mara"}},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			message := text(t, failure, "message")
			if !strings.Contains(message, "simple auth login") {
...
func TestReadOnlyToolsRunWithoutACredential(t *testing.T) {
	...
	for _, name := range []string{"simple_status", "simple_domain_list", "simple_record_list", "simple_site_show", "simple_version"} {
		arguments := map[string]any{}
		if name == "simple_record_list" || name == "simple_site_show" {
...
func TestToolsWithoutAUsableControlPlaneRefuseRatherThanCrash(t *testing.T) {
	...
	failure := refusal(t, client.call("simple_domain_list", nil))
	if failure["kind"] != KindNotConfigured {
		t.Errorf("kind = %v, want %s", failure["kind"], KindNotConfigured)
	}

	// simple_version answers from this process, so it still works: an operator
	// with a broken configuration can always find out what the server thinks.
	answer := client.call("simple_version", nil)
	if answer.IsError {
		t.Fatalf("simple_version was refused: %v", answer.StructuredContent)
	}
	if answer.StructuredContent["control_plane_usable"] != false {
		t.Errorf("simple_version must admit the control plane is unusable, got %v", answer.StructuredContent)
	}
...
func TestUnknownToolAndMalformedInput(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "unknown tool", request: `{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"simple_launch_missiles"}}`, want: KindUnknownTool},
		{name: "no tool name", request: `{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{}}`, want: KindInvalidArgument},
		{name: "arguments are not an object", request: `{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"simple_status","arguments":[1]}}`, want: KindInvalidArgument},
