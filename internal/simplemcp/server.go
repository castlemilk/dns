// Package simplemcp serves the `simple` platform over the Model Context
// Protocol's stdio transport: newline-delimited JSON-RPC on stdin and stdout,
// with one tool per operation the `simple` CLI performs.
//
...
func (s *Server) permit(definition tool, arguments args) error {
	...
	if s.clients == nil {
		return &toolError{
			...
			Message: fmt.Sprintf("this server has no usable control plane: %s. Set SIMPLE_API_URL, or pass --api-url, and restart the MCP server", s.unusable),
		}
	}
	if definition.Mutates && !s.credentialed {
		message := fmt.Sprintf("%s changes %s and no operator credential is configured. Run `simple auth login`, or set SIMPLE_API_TOKEN in this server's environment, and restart it. Read-only tools still work.",
