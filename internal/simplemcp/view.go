...
func (s *Server) findZone(ctx context.Context, a args) (*dnsv1.Zone, error) {
	...
	if id != "" {
		return nil, notFound("zone_id: no domain with that id is hosted here; call simple_domain_list to see the domains that are")
	}
	return nil, notFound("domain: " + name + " is not hosted here; call simple_domain_list to see the domains that are")
}
