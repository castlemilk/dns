...
func TestMailboxPasswordIsReturnedOnceWithItsInstruction(t *testing.T) {
	...
	answer := client.call("simple_mail_mailbox_add", map[string]any{"domain": "acme.dev", "local_part": "mara"})
	if answer.IsError {
	...
	for _, phrase := range []string{"once", "keep no copy", "simple_mail_mailbox_reset"} {
	...
	definition := toolNamed(t, "simple_mail_mailbox_add")
	...
	reset := client.call("simple_mail_mailbox_reset", map[string]any{"domain": "acme.dev", "address": "mara@acme.dev"})
	if reset.IsError {
...
func TestSecretsNeverComeBack(t *testing.T) {
	...
	requests := strings.Join([]string{
		`{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"simple_site_env_set","arguments":{"domain":"acme.dev","name":"DATABASE_URL","value":"` + value + `"}}}`,
		`{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"simple_site_env_get","arguments":{"domain":"acme.dev"}}}`,
		`{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"simple_version","arguments":{}}}`,
...
func TestAnEngineThatIsNotConfiguredIsReportedVerbatim(t *testing.T) {
	...
	failure := refusal(t, client.call("simple_site_list", nil))
...
func TestARejectedCredentialPointsAtLoggingInAgain(t *testing.T) {
	...
	if message := text(t, failure, "message"); !strings.Contains(message, "simple auth login") {
...
func TestUnbindingMailboxesNeedsTheDomainNameTyped(t *testing.T) {
	...
	failure := refusal(t, client.call("simple_mail_unbind", map[string]any{
	...
	answer := client.call("simple_mail_unbind", map[string]any{
...
func TestDeletingAMailboxCarriesTheAddressAsConfirmation(t *testing.T) {
	...
	failure := refusal(t, client.call("simple_mail_mailbox_delete", map[string]any{
	...
	answer := client.call("simple_mail_mailbox_delete", map[string]any{
...
func TestFalseFieldsThatMatterAreNotOmitted(t *testing.T) {
	...
	answer := client.call("simple_status", nil)
	if answer.IsError {
		t.Fatalf("the call was refused: %v", answer.StructuredContent)
	}
...
func TestLargeCountsSurviveAsStrings(t *testing.T) {
	...
	answer := client.call("simple_mail_mailbox_list", map[string]any{"domain": "acme.dev"})
	if answer.IsError {
		t.Fatalf("the call was refused: %v", answer.StructuredContent)
	}
