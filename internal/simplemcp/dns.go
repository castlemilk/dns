...
func (s *Server) domainAdd(ctx context.Context, a args) (any, error) {
	...
	return viewWith(response.Msg, map[string]any{
		"note": "The zone exists here. Traffic only reaches it once the domain's registrar delegates it to the nameservers simple_status reports.",
	})
}
...
func (s *Server) recordEdit(ctx context.Context, a args) (any, error) {
	...
	if current == nil {
		return nil, notFound("record_id: " + zone.GetName() + " has no record with that id; call simple_record_list to see the ones it has")
	}
