...
import (
	...
	"github.com/castlemilk/dns/internal/deephostcli/auth"
	"github.com/castlemilk/dns/internal/deephostcli/client"
	"github.com/castlemilk/dns/internal/deephostcli/config"
)

// envConsoleURL points `deephost auth login` at the console when it is not served
// from the control plane's own origin.
const envConsoleURL = "DEEPHOST_CONSOLE_URL"
...
func cmdAuthLogin(ctx context.Context, e *env, o *options, args []string) error {
	var noBrowser bool
	fs, err := o.parse("deephost auth login", authUsage, args, func(fs *flag.FlagSet) {
...
// fallback; --console-url or DEEPHOST_CONSOLE_URL covers a split deployment.
...
func cmdAuthLogout(e *env, o *options, args []string) error {
	fs, err := o.parse("deephost auth logout", authUsage, args, nil)
	if err != nil {
		return err
	}
	if err := noExtraArgs(fs, 0, authUsage); err != nil {
...
func cmdAuthStatus(ctx context.Context, e *env, o *options, args []string) error {
	var check bool
	fs, err := o.parse("deephost auth status", authUsage, args, func(fs *flag.FlagSet) {
...
func credentialSummary(resolved config.Resolved) string {
	if resolved.Token == "" {
		return "none; run `deephost auth login`, or pass --token"
	}
...
func cmdAuthToken(e *env, o *options, args []string) error {
	var show bool
	fs, err := o.parse("deephost auth token", authUsage, args, func(fs *flag.FlagSet) {
	...
	if resolved.Token == "" {
		return errors.New("token: none is configured; run `deephost auth login`, or pass --token")
	}
	if !show {
		return newPrinter(e, o).emit(struct {
			...
			return fields(w,
				...
				pair("to print it", "deephost auth token --show"),
			)
		})
	}
