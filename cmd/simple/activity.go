...
func cmdActivity(ctx context.Context, e *env, o *options, args []string) error {
	if len(args) > 0 {
		...
		case "list":
			// `simple activity list` reads naturally even though the bare
			// group already lists.
			args = args[1:]
		}
	}
...
func cmdActivityList(ctx context.Context, e *env, o *options, args []string) error {
	...
	var limit uint
	var cursor string
	fs, err := o.parse("simple activity", activityUsage, args, func(fs *flag.FlagSet) {
...
func cmdActivityShow(ctx context.Context, e *env, o *options, args []string) error {
	fs, err := o.parse("simple activity show", activityUsage, args, nil)
	if err != nil {
		return err
	}
