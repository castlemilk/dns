...
func TestExitCodes(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "no arguments greet", args: nil, want: codeOK, stdout: "usage: simple"},
		{name: "help", args: []string{"help"}, want: codeOK, stdout: "groups:"},
		{name: "help flag", args: []string{"--help"}, want: codeOK, stdout: "usage: simple"},
		...
		},
		{
			...
			want: codeOK, stdout: "simple site attach",
		},
		{
...
// TestGlobalFlagsAreAcceptedBeforeAndAfterTheCommand keeps `simple --json
// domain list` and `simple domain list --json` the same command.
	...
	t.Run("every group answers --json", func(t *testing.T) {
		...
		for _, command := range commands {
			...
			if code := h.run(args...); code != codeOK {
				t.Errorf("simple %s: exit %d\n%s", strings.Join(args, " "), code, h.err())
				continue
			}
			if strings.TrimSpace(h.out()) == "" {
				t.Errorf("simple %s wrote no document", strings.Join(args, " "))
				continue
			}
			h.decode()
		}
...
func TestTextOutputIsBrandedAndPlain(t *testing.T) {
	...
	if !strings.HasPrefix(h.out(), "simple\n") {
	...
	t.Run("every destructive command asks", func(t *testing.T) {
		...
		for _, command := range destructive {
			...
			if code := h.run(command...); code != codeError {
				t.Errorf("simple %s: exit %d, want a refusal", strings.Join(command, " "), code)
				continue
			}
			contains(t, h.err(), "[y/N]", "simple "+strings.Join(command, " ")+" prompt")
		}
	})
}
...
func TestCredentialTravelsOnTheHeader(t *testing.T) {
	...
	contains(t, h.err(), "run `simple auth login`", "the rejection")
...
func TestHelpIsADocumentInJSONMode(t *testing.T) {
	...
	for _, args := range [][]string{
		...
		if code := h.run(args...); code != codeOK {
			t.Fatalf("simple %s: exit %d", strings.Join(args, " "), code)
		}
		if usage := field[string](t, h.decode(), "usage"); !strings.Contains(usage, "usage: simple") {
			t.Fatalf("simple %s: usage = %q", strings.Join(args, " "), usage)
		}
	}
}
