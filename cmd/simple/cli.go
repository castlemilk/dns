...
type env struct {
	...
	// now is the clock `simple activity --since 2h` reads. Nil means
	// time.Now.
	now func() time.Time

	// version is what `simple version` reports. Empty means the build info
...
// top-level flag set and again on each command's, so `simple --json domain
// list` and `simple domain list --json` mean the same thing.
type options struct {
	apiURL     string
	consoleURL string
	token      string
...
func (o *options) register(fs *flag.FlagSet) {
	fs.StringVar(&o.apiURL, "api-url", o.apiURL, "control plane base URL (SIMPLE_API_URL)")
	fs.StringVar(&o.token, "token", o.token, "operator credential (SIMPLE_API_TOKEN)")
	fs.StringVar(&o.consoleURL, "console-url", o.consoleURL, "console base URL for `simple auth login` (SIMPLE_CONSOLE_URL)")
...
// `simple record add example.com --type A` silently treat --type as a
...
func run(ctx context.Context, args []string, e *env) int {
	...
	head := flag.NewFlagSet("simple", flag.ContinueOnError)
	...
	if len(rest) == 0 {
		// `simple` with no arguments is the branded greeting, not a mistake.
...
// greet prints the banner and the command list. It is what `simple` alone does.
...
func report(e *env, o *options, err error) int {
	...
	if _, writeErr := fmt.Fprintf(e.stderr, "simple: %v\n", err); writeErr != nil {
		return codeError
	}
