...
// envGitToken carries a private repository's token into `simple site deploy`
// without putting it in the process list.
const envGitToken = "SIMPLE_GIT_TOKEN" //nolint:gosec // the name of a variable, not a credential
...
func cmdSiteList(ctx context.Context, e *env, o *options, args []string) error {
	fs, err := o.parse("simple site list", siteUsage, args, nil)
	if err != nil {
		return err
	}
	...
	return newPrinter(e, o).emit(sites, func(w io.Writer) error {
		if len(sites.GetSites()) == 0 {
			return writeLine(w, "no sites yet; attach one with `simple site attach DOMAIN --framework static`")
		}
...
func cmdSiteShow(ctx context.Context, e *env, o *options, args []string) error {
	fs, err := o.parse("simple site show", siteUsage, args, nil)
	if err != nil {
		return err
	}
...
func cmdSiteAttach(ctx context.Context, e *env, o *options, args []string) error {
	...
	fs, err := o.parse("simple site attach", siteUsage, args, func(fs *flag.FlagSet) {
...
func cmdSiteDetach(ctx context.Context, e *env, o *options, args []string) error {
	...
	fs, err := o.parse("simple site detach", siteUsage, args, func(fs *flag.FlagSet) {
...
func cmdSiteDeploy(ctx context.Context, e *env, o *options, args []string) error {
	...
	fs, err := o.parse("simple site deploy", siteUsage, args, func(fs *flag.FlagSet) {
	...
	return newPrinter(e, o).emit(response.Msg, func(w io.Writer) error {
		...
		return writeLine(w, "\nfollow it with `simple site logs "+site.GetZoneName()+" --deploy-id "+deploy.GetId()+"`")
	})
}
...
func cmdSiteDeploys(ctx context.Context, e *env, o *options, args []string) error {
	...
	fs, err := o.parse("simple site deploys", siteUsage, args, func(fs *flag.FlagSet) {
...
func cmdSiteRollback(ctx context.Context, e *env, o *options, args []string) error {
	var deployID string
	fs, err := o.parse("simple site rollback", siteUsage, args, func(fs *flag.FlagSet) {
	...
	if strings.TrimSpace(deployID) == "" {
		return usagef("--deploy-id: is required; list them with `simple site deploys DOMAIN`")
	}
	clients, err := o.connect(e)
	if err != nil {
		return err
...
func cmdSiteLogs(ctx context.Context, e *env, o *options, args []string) error {
	var deployID string
	var tail uint
	fs, err := o.parse("simple site logs", siteUsage, args, func(fs *flag.FlagSet) {
...
func cmdSiteEnvGet(ctx context.Context, e *env, o *options, args []string) error {
	...
	fs, err := o.parse("simple site env get", siteUsage, args, func(fs *flag.FlagSet) {
...
func cmdSiteEnvSet(ctx context.Context, e *env, o *options, args []string) error {
	...
	fs, err := o.parse("simple site env set", siteUsage, args, func(fs *flag.FlagSet) {
...
func cmdSiteEnvUnset(ctx context.Context, e *env, o *options, args []string) error {
	var environment string
	fs, err := o.parse("simple site env unset", siteUsage, args, func(fs *flag.FlagSet) {
		fs.StringVar(&environment, "environment", "", "production, preview or development (default production)")
	})
	if err != nil {
		return err
...
func cmdSiteUpdate(ctx context.Context, e *env, o *options, args []string) error {
	...
	fs, err := o.parse("simple site update", siteUsage, args, func(fs *flag.FlagSet) {
		fs.StringVar(&repository, "repository", "", "https://github.com/owner/name")
...
func cmdSiteWww(ctx context.Context, e *env, o *options, args []string) error {
	var mode string
	fs, err := o.parse("simple site www", siteUsage, args, func(fs *flag.FlagSet) {
...
func cmdSiteReapplyDNS(ctx context.Context, e *env, o *options, args []string) error {
	...
	fs, err := o.parse("simple site reapply-dns", siteUsage, args, func(fs *flag.FlagSet) {
		fs.BoolVar(&replaceConflicting, "replace-conflicting", false, "replace records that block the site's own")
...
func cmdSiteGateway(ctx context.Context, e *env, o *options, args []string) error {
	...
	fs, err := o.parse("simple site gateway", siteUsage, args, func(fs *flag.FlagSet) {
...
func lookupSite(ctx context.Context, o *options, clients *client.Clients, reference string) (*hostingv1.Site, error) {
	...
	if site == nil {
		return nil, fmt.Errorf("domain: %s has no site; attach one with `simple site attach %s --framework static`",
