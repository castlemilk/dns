...
func TestRunUsage(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "version", arguments: []string{"--version"}, want: exitOK, wantOut: "simple-mcp "},
...
func TestRunServesTheProtocolOnStdoutAndTheReportOnStderr(t *testing.T) {
	t.Parallel()
	home := t.TempDir()
	getenv := environment(map[string]string{"HOME": home, config.EnvAPIURL: "https://api.simple.test"})
	...
	if err := config.Save(path, config.Config{APIURL: "https://stored.simple.test", Token: token}); err != nil {
		t.Fatal(err)
	}

	request := `{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"simple_version","arguments":{}}}` + "\n"
	var stdout, stderr strings.Builder
	...
	if !strings.Contains(stdout.String(), "https://api.simple.test") {
...
func TestRunStartsWithoutACredentialAndSaysSo(t *testing.T) {
	...
	request := `{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"simple_domain_add","arguments":{"name":"acme.dev"}}}` + "\n"
...
func TestRunSurvivesAnUnreadableSettingsFile(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(`{"api_url":"https://api.simple.test","token":"exposed"}`), 0o644); err != nil {
		t.Fatal(err)
	}
