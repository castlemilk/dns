# `simple` — the command line client

`simple` is one binary that does everything the console does: zones and
...
There is nothing else to install. `simple` links only the standard library and
...
| --- | --- | --- | --- |
| Control plane URL | `--api-url` | `SIMPLE_API_URL` | `api_url` |
| Operator credential | `--token` | `SIMPLE_API_TOKEN` | `token` |
| Console URL (login only) | `--console-url` | `SIMPLE_CONSOLE_URL` | — |
...
The file is `~/.config/simple/config.json` (`XDG_CONFIG_HOME` is honoured),
...
The credential is never printed. It is redacted in `simple auth status`, in
every error sentence, and in the URL `simple auth login` opens. `simple auth
token --show` is the single exception, and it warns on stderr first.

## Authentication

```sh
simple auth login                 # hand the credential over from an unlocked console
simple auth login --no-browser    # print the approval URL instead of opening one
simple auth logout                # forget the stored credential
simple auth status [--check]      # where the URL and credential in force came from
simple auth token --show          # print the credential (warns first)
```
...
`simple auth login` is an authorization-code-shaped *handoff* of that one
...
1. `simple` generates a PKCE verifier and its S256 challenge and a random
...
3. Approve POSTs the state, the challenge and the token to the loopback
   callback. `simple` checks the state and the challenge against what it
   generated, writes `0600`, and prints the banner. Deny POSTs
   `access_denied`, and `simple` exits without writing anything.
...
- **There is no per-CLI revocation.** `simple auth logout` deletes this
...
flow and set `SIMPLE_API_TOKEN`, or pass `--token`.
...
--console-url URL  console base URL, for `simple auth login`
...
`simple record add example.com --type A` and
`simple --json record add example.com --type A` both work.
...
prose, warnings and progress go to stderr, so `simple … --json | jq` is always
safe.
...
`simple record export` writes a zone file, and `simple site logs` writes log
...
```text
simple: domain: is required
simple: record_id: "abc" is not a record in example.test
```
...
| --- | --- |
| `unauthenticated` | *&lt;url&gt; refused the credential; run `simple auth login`, or pass --token* |
...
```text
$ simple record delete example.test 7942a199…
Delete www A "203.0.113.10" from example.test? [y/N]: n
simple: cancelled; nothing was changed
```
...
`simple`, `simple version` and a successful `simple auth login` print the
...
```sh
simple domain list
simple domain add example.com
simple domain show example.com          # name, id, trailing dot — any of them
simple domain delete example.com --yes
```
...
```sh
simple record list example.com [--type A] [--name www]
simple record add example.com --name www --type A --value 203.0.113.10 --ttl 300
simple record edit example.com RECORD_ID --value 203.0.113.11
simple record delete example.com RECORD_ID --yes
simple record import example.com --file zone.txt --mode replace --dry-run
simple record export example.com --output zone.txt
```
...
`delete` — change them through `simple site` or `simple mail` instead.
...
```sh
simple site attach example.com --framework nextjs --repository https://github.com/acme/site --dry-run
simple site attach example.com --framework nextjs --repository https://github.com/acme/site
simple site deploy example.com --revision main
simple site deploys example.com --limit 5
simple site logs example.com --tail 200
simple site rollback example.com --deploy-id dpl_123
simple site env get example.com
printf %s "$DATABASE_URL" | simple site env set example.com DATABASE_URL
simple site env unset example.com DATABASE_URL --yes
simple site www example.com --mode redirect
simple site detach example.com --yes
```
...
the same reason `simple site deploy` takes a private-repository token from
`SIMPLE_GIT_TOKEN` or `--git-token-stdin`; `--git-token` is refused as a usage
error.
...
```sh
simple mail bind example.com --dmarc-policy quarantine --dry-run
simple mail bind example.com --dmarc-policy quarantine
simple mail show example.com
simple mail mailbox add example.com ben --display-name "Ben"
simple mail mailbox reset example.com ben@example.com --yes
simple mail forwarder add example.com hello --target ben@example.com
simple mail queue example.com
simple mail unbind example.com --delete-mailboxes --yes
```
...
Where the mail engine could not vouch for a count, `simple` prints `unknown`
...
simple billing subscribe example.com --open
simple billing portal --flow payment_method_update
simple billing invoices --limit 20
```
...
```sh
simple activity --domain example.com --kind deploy. --since 24h
simple activity show 000001a07068bfd2-0000
```
...
```sh
simple platform rebuild --dry-run
simple platform rebuild --yes
```
